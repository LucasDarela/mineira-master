          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <input type="text" name="name" defaultValue={staffMember.name} required className="w-full p-2 border rounded text-sm" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase">Cargo</label>
                <input type="text" name="role" defaultValue={staffMember.role} required className="w-full p-2 border rounded text-sm" placeholder="Ex: Treinador" />
              </div>
              <div>
  ...
  return (
    <tr className="hover:bg-gray-50 transition-colors">
      <td className="p-3 font-semibold text-[#001f3f] flex items-center gap-3">
        <img src={staffMember.image} alt={staffMember.name} className="w-8 h-8 rounded-full object-cover border" />
        {staffMember.name}
      </td>
      <td className="p-3 text-sm text-gray-600">{staffMember.role}</td>
